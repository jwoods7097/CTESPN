...
FROM MLS.MatchClubPlayer MCP
    INNER JOIN MLS.Match M ON MCP.MatchID = M.MatchID
    INNER JOIN HomeClub ON HomeClub.MatchID = M.MatchID
    LEFT JOIN AwayClub ON AwayClub.MatchID = M.MatchID
WHERE (HomeClub.ClubID = @OpponentClubID OR AwayClub.ClubID = @OpponentClubID) AND MCP.PlayerID = @PlayerID AND MCP.Played = 1 AND M.[Date] BETWEEN @StartDate AND @EndDate
ORDER BY M.[Date] ASC
GO
